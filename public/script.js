...
async function fetchNewArticles() {
  try {
    const feedsData = await import_promises.default.readFile("./data/feed.json", "utf-8");
    const feeds = JSON.parse(feedsData);
    const newArticles = [];
    for (const feed of feeds) {
      const rssResponse = await fetch(feed.rssUrl);
    ...
    await import_promises.default.writeFile("./data/articles.json", JSON.stringify(newArticles, null, 2));
    return newArticles;
  } catch (error) {
...
function isNewArticle(pubDate, title) {
  ...
  try {
    const data = import_promises.default.readFileSync("./data/articles.json", "utf-8");
    if (data) {
